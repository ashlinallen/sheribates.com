            ...
            function gotoPage(newPage, time) {
                ...
                if (!TweenMax.isTweening($slider) && !TweenMax.isTweening($items)) {
                    ...
                    case "slide":
                        ...
                        TweenMax.to($slider, time, {ease:Sine.easeInOut, css: {left: left}, onComplete: endHandler, onCompleteParams: [time]});
                        break;
                    case "fade":
                        $items.eq(previousItem).css("zIndex", 1);
                        TweenMax.to($items.eq(currentItem).css("zIndex", 2), time, {ease:Sine.easeOut, css: {autoAlpha: 1}, onComplete: endHandler, onCompleteParams: [time, $items.eq(previousItem)]});
                        break;
                    }
                }
            }
